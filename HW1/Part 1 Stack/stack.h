...
{ 
 ...
 public:  // available to the client
  ...
  //          otherwise, throws an exception - Underflow
  // PARAMETER: ** provide variable to receive the top element by reference
  void topElem(el_t&);
  
  // ** Must add good comments for each function - See Notes1B

  //PURPOSE: ** to provide a value if the stack is empty
  bool isEmpty();

  //PURPOSE: ** to provide a value if the stack is full
  bool isFull();

  //PURPOSE: ** to display every element in the stack
  void displayAll();

  //PURPOSE: ** to empty the stack
  void clearIt();
