...
//PURPOSE: To find an object in the linked list based on the key object
...
//            If node's element is not equal to give key continue looping until end of list else return
//            blank object.
el_t slist::search2(el_t key)
{
  Node *p = Front; //temporary pointer to front of list
  el_t blank; // blank object to return when key cannot be found
  while(p != NULL) //keep looping until p pointer is NULL
    {
      if(p->Elem == key) //check is node's element is equal to given key.
        {
          return p->Elem; //If node's element is equal to key, return elemNum(position)
        }
      p = p->Next; //set p pointer to p->Next pointer
    }
  return blank; // return balnk object
}
